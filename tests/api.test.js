...
(async () => {
  ...
  const duplicateOtp = await request('/api/phone/request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: '+919999999999' }) }); assert.equal(duplicateOtp.status, 200);
  const duplicatePair = await request('/api/phone/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: '+919999999999', code: duplicateOtp.json.demoCode }) }); assert.equal(duplicatePair.status, 409); assert.equal(duplicatePair.json.locked, true); assert.equal(duplicatePair.json.deviceLinkId, paired.json.deviceLinkId);
