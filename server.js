...
async function handler(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/phone/verify' && req.method === 'POST') {
      ...
      const data = await readData();
      const existingLink = data.deviceLinks.find(item => item.phone === phone);
      if (existingLink) {
        pendingPhoneOtps.delete(phone);
        return send(res, 409, { paired: true, locked: true, deviceLinkId: existingLink.id, phone, error: 'This phone is already paired and cannot be disconnected or paired again. An admin must remove it from the dashboard before it can be paired again.' });
      }
      const vehicle = pending.vehicleId ? data.vehicles.find(item => item.id === pending.vehicleId) : null; const deviceToken = createDeviceToken(); const deviceLink = { id: `DL-${Date.now()}`, phone, vehicleId: vehicle?.id || null, userId, mode: pending.mode, pairedAt: new Date().toISOString(), pairingLocked: true, status: 'Stopped', speed: 0, lastSeen: null, lastPosition: null, trackerType: 'car', cameraEnabled: false, cameraFacing: 'back', micEnabled: false, deviceTokenHash: hashDeviceToken(deviceToken) }; data.deviceLinks.push(deviceLink); data.deviceLinks = data.deviceLinks.slice(-20); audit(data, `Paired ${maskPhone(phone)} as a permanent mobile device${vehicle ? ` for ${vehicle.name}` : ''}.`); await writeData(data); pendingPhoneOtps.delete(phone); return send(res, 200, { paired: true, locked: true, deviceLinkId: deviceLink.id, deviceToken, vehicleId: vehicle?.id || null, vehicleName: vehicle?.name || null, phone: deviceLink.phone, mode: pending.mode, userId });
    }
    ...
    if (pathname === '/api/device/config' && req.method === 'GET') {
      ...
      return send(res, 200, { micEnabled: Boolean(link.micEnabled), cameraEnabled: Boolean(link.cameraEnabled), cameraFacing: link.cameraFacing || 'back' });
    }
