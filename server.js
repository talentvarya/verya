...
function normalizeData(data) {
  ...
  data.alertPolicies ||= {};
  data.deviceLinks ||= [];
  data.deviceLinks.forEach(link => { link.activationStatus ||= 'idle'; link.trackingApproved = link.activationStatus === 'approved'; });
...
function acceptedDistanceMeters(a, b) {
...
function speedBand(speed) { const start = Math.floor(Math.max(0, Number(speed) || 0) / 10) * 10; return { start, end: start + 9, label: `${start}–${start + 9} km/h` }; }
function dailySpeedReports(data) { const samples = new Map(); (data.events || []).forEach(event => { const raw = event.raw || {}; const lat = Number(raw.lat); const lng = Number(raw.lng); if (!Number.isFinite(lat) || !Number.isFinite(lng)) return; const key = event.deviceLinkId || event.vehicleId; if (!key) return; if (!samples.has(key)) samples.set(key, []); samples.get(key).push({ key, lat, lng, accuracy: Number(raw.accuracy || 20), speed: Number(raw.speed || 0), receivedAt: new Date(event.receivedAt || 0).getTime() }); }); const totals = new Map(); samples.forEach(items => { items.sort((a, b) => a.receivedAt - b.receivedAt); for (let index = 1; index < items.length; index += 1) { const previous = items[index - 1]; const current = items[index]; const gap = current.receivedAt - previous.receivedAt; if (gap <= 0 || gap > 10 * 60 * 1000) continue; const day = localDayKey(previous.receivedAt, data.settings?.timezone || 'Asia/Calcutta'); const band = speedBand(previous.speed); const rowKey = `${day}|${previous.key}|${band.start}`; const row = totals.get(rowKey) || { day, deviceLinkId: previous.key, band: band.label, speedMin: band.start, minutes: 0, meters: 0 }; row.minutes += Math.min(gap, 10 * 60 * 1000) / 60000; const step = acceptedDistanceMeters(previous, current); if (step !== null && step > 0) row.meters += step; totals.set(rowKey, row); } }); return [...totals.values()].map(row => ({ ...row, minutes: Number(row.minutes.toFixed(1)), km: Number((row.meters / 1000).toFixed(2)) })).sort((a, b) => b.day.localeCompare(a.day) || a.deviceLinkId.localeCompare(b.deviceLinkId) || a.speedMin - b.speedMin); }
...
async function handler(req, res) {
  ...
  try {
    ...
    const publicApi = pathname === '/api/health' || pathname === '/api/phone/request' || pathname === '/api/phone/verify' || pathname === '/api/device/config' || pathname === '/api/device/activation' || pathname === '/api/ingest/position' || pathname === '/api/ingest/camera' || pathname === '/api/ingest/mic';
    ...
    if (pathname === '/api/phone/verify' && req.method === 'POST') {
      ...
      const data = await readData(); const vehicle = pending.vehicleId ? data.vehicles.find(item => item.id === pending.vehicleId) : null; const deviceToken = createDeviceToken(); const deviceLink = { id: `DL-${Date.now()}`, phone, vehicleId: vehicle?.id || null, userId, mode: pending.mode, pairedAt: new Date().toISOString(), status: 'Stopped', speed: 0, lastSeen: null, lastPosition: null, trackerType: 'car', activationStatus: 'idle', trackingApproved: false, cameraEnabled: false, cameraFacing: 'back', micEnabled: false, deviceTokenHash: hashDeviceToken(deviceToken) }; data.deviceLinks.push(deviceLink); data.deviceLinks = data.deviceLinks.slice(-20); audit(data, `Paired ${maskPhone(phone)} as a mobile device${vehicle ? ` for ${vehicle.name}` : ''}. Waiting for tracking consent.`); await writeData(data); pendingPhoneOtps.delete(phone); return send(res, 200, { paired: true, deviceLinkId: deviceLink.id, deviceToken, vehicleId: vehicle?.id || null, vehicleName: vehicle?.name || null, phone: deviceLink.phone, mode: pending.mode, userId, activationStatus: deviceLink.activationStatus });
    }
    const deviceMatch = pathname.match(/^\/api\/devices\/([^/]+)$/);
    const activationDeviceMatch = pathname.match(/^\/api\/devices\/([^/]+)\/activation$/);
    if (activationDeviceMatch && req.method === 'POST') {
      const data = await readData(); if (!can(data, 'manageVehicles')) return deny(res, 'request tracking activation');
      const link = data.deviceLinks.find(item => item.id === decodeURIComponent(activationDeviceMatch[1])); if (!link) return send(res, 404, { error: 'Device not found.' });
      link.activationStatus = 'pending'; link.trackingApproved = false; link.activationRequestId = `AR-${Date.now()}`; link.activationRequestedAt = new Date().toISOString(); link.activationRequestedBy = req.authUser?.email || 'Admin';
      audit(data, `Requested tracking consent from ${maskPhone(link.phone)}.`); await writeData(data); return send(res, 200, { deviceLinkId: link.id, activationStatus: link.activationStatus, activationRequestId: link.activationRequestId });
    }
    if (deviceMatch && req.method === 'PATCH') {
      ...
      if (input.cameraFacing !== undefined) { if (!['front', 'back'].includes(input.cameraFacing)) return send(res, 400, { error: 'Camera must be front or back.' }); link.cameraFacing = input.cameraFacing; audit(data, `Selected ${input.cameraFacing} camera for ${link.phone}.`); }
      if (input.cameraEnabled !== undefined) { link.cameraEnabled = Boolean(input.cameraEnabled); audit(data, `${link.cameraEnabled ? 'Enabled' : 'Disabled'} camera for ${link.phone}.`); if (!link.cameraEnabled) cameraFrames.delete(link.id); }
    ...
    if (deviceMatch && req.method === 'DELETE') {
      ...
      const [removed] = data.deviceLinks.splice(index, 1); microphoneFrames.delete(removed.id); cameraFrames.delete(removed.id); audit(data, `Removed mobile device ${removed.phone}.`); await writeData(data); return send(res, 200, { deleted: true, deviceLinkId: removed.id });
    }
    if (pathname === '/api/device/config' && req.method === 'GET') {
      ...
      return send(res, 200, { micEnabled: Boolean(link.micEnabled), cameraEnabled: Boolean(link.cameraEnabled), cameraFacing: link.cameraFacing || 'back', activationStatus: link.activationStatus || 'idle', trackingApproved: link.activationStatus === 'approved', activationRequestId: link.activationRequestId || null });
    }
    if (pathname === '/api/device/activation' && req.method === 'GET') {
      const query = new URL(req.url, `http://${req.headers.host || 'localhost'}`).searchParams; const data = await readData(); const link = data.deviceLinks.find(item => item.id === query.get('deviceLinkId'));
      if (!link || !deviceTokenMatches(req, {}, link)) return send(res, 401, { error: 'This device is not authorized.' });
      return send(res, 200, { deviceLinkId: link.id, activationStatus: link.activationStatus || 'idle', activationRequestId: link.activationRequestId || null, vehicleName: data.vehicles.find(vehicle => vehicle.id === link.vehicleId)?.name || null });
    }
    if (pathname === '/api/device/activation' && req.method === 'POST') {
      const input = await body(req); const data = await readData(); const link = data.deviceLinks.find(item => item.id === input.deviceLinkId);
      if (!link || !deviceTokenMatches(req, input, link)) return send(res, 401, { error: 'This device is not authorized.' });
      const decision = String(input.decision || '').toLowerCase(); if (!['approve', 'reject'].includes(decision)) return send(res, 400, { error: 'Choose approve or reject.' });
      link.activationStatus = decision === 'approve' ? 'approved' : 'rejected'; link.trackingApproved = decision === 'approve'; link.activationRespondedAt = new Date().toISOString();
      audit(data, `${decision === 'approve' ? 'Approved' : 'Rejected'} tracking consent for ${maskPhone(link.phone)}.`); await writeData(data); return send(res, 200, { deviceLinkId: link.id, activationStatus: link.activationStatus, trackingApproved: link.trackingApproved });
    }
    ...
    if (pathname === '/api/ingest/position' && req.method === 'POST') {
      const data = await readData(); if (!can(data, 'ingest')) return deny(res, 'ingest position events'); const input = await body(req); const link = input.deviceLinkId ? data.deviceLinks.find(item => item.id === input.deviceLinkId) : null; if (link && !deviceTokenMatches(req, input, link)) return send(res, 401, { error: 'This device is not authorized. Pair it again.' }); if (link && link.activationStatus !== 'approved') return send(res, 403, { error: 'Tracking is waiting for approval on the mobile device.' }); const vehicle = data.vehicles.find(item => item.id === input.vehicleId) || (link?.vehicleId ? data.vehicles.find(item => item.id === link.vehicleId) : null); if (!vehicle && !link) return send(res, 404, { error: 'Vehicle or paired device not found.' });
    ...
    if (pathname === '/api/ingest/camera' && req.method === 'POST') {
      ...
      if (!link.cameraEnabled) return send(res, 403, { error: 'Camera is disabled by the admin.' });
