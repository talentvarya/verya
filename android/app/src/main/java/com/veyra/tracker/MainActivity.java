...
public class MainActivity extends Activity {
    ...
    private EditText phone, otp; private TextView status; private Button start, camera, microphone; private String pendingPhoneValue, pendingOtpCode; private boolean verifyInProgress;
    private SharedPreferences prefs;

    @Override public void onCreate(Bundle state) { super.onCreate(state); prefs = getSharedPreferences(PREFS, MODE_PRIVATE); buildUi(); if (paired()) { start.setEnabled(true); camera.setEnabled(true); microphone.setEnabled(true); } }
    private void buildUi() {
        ...
        TextView info = new TextView(this); info.setText("Pair this phone with your Veyra account. Location, camera and microphone sharing require permission and stay visible in Android notifications."); root.addView(info);
        ...
        microphone = button("Allow microphone"); root.addView(microphone); microphone.setEnabled(false); microphone.setOnClickListener(v -> enableMicrophone());
    ...
    private void verifyPairing() { if (verifyInProgress) return; final String value = phoneValue(); if (value == null) return; pendingPhoneValue = value; pendingOtpCode = otp.getText().toString().trim(); if (!mediaPermissionsReady()) { status.setText("Camera and microphone permission is required before verification."); if (Build.VERSION.SDK_INT >= 23) requestPermissions(new String[]{Manifest.permission.CAMERA, Manifest.permission.RECORD_AUDIO}, 25); else completeVerifyPairing(); return; } completeVerifyPairing(); }
    ...
    private void completeVerifyPairing() { if (verifyInProgress || pendingPhoneValue == null) return; verifyInProgress = true; status.setText("Permissions allowed. Verifying device…"); try { JSONObject body = new JSONObject(); body.put("phone", pendingPhoneValue); body.put("code", pendingOtpCode); post("/api/phone/verify", body, result -> { verifyInProgress = false; prefs.edit().putString("device_token", result.getString("deviceToken")).putString("device_link_id", result.getString("deviceLinkId")).putBoolean("camera_permission_ready", true).putBoolean("mic_permission_ready", true).apply(); runOnUiThread(() -> { start.setEnabled(true); camera.setEnabled(true); microphone.setEnabled(true); status.setText("Paired. Camera and microphone permissions are ready. Start tracking when needed."); requestLocationPermissions(); }); }); } catch (Exception e) { verifyInProgress = false; status.setText(e.getMessage()); } }
    ...
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] results) { super.onRequestPermissionsResult(requestCode, permissions, results); if (requestCode == 25) { if (mediaPermissionsReady()) completeVerifyPairing(); else { verifyInProgress = false; status.setText("Both camera and microphone permissions are required. Verification cancelled."); } } if ((requestCode == 21 || requestCode == 22) && locationReady()) { status.setText("Location allowed. Starting background tracking…"); startTracking(); } if (requestCode == 23 && checkSelfPermission(Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) enableCamera(); if (requestCode == 24 && checkSelfPermission(Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED) enableMicrophone(); }
