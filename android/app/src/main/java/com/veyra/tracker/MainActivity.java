...
public class MainActivity extends Activity {
    ...
    private EditText phone, otp; private TextView status; private Button start;
    private SharedPreferences prefs;

    @Override public void onCreate(Bundle state) { super.onCreate(state); prefs = getSharedPreferences(PREFS, MODE_PRIVATE); buildUi(); if (paired()) start.setEnabled(true); }
    private void buildUi() {
        ...
        TextView info = new TextView(this); info.setText("Pair this phone with your Veyra account. Android will show a notification while tracking is active."); root.addView(info);
        phone = field("Mobile number, e.g. +919876543210"); root.addView(phone);
        ...
        Button stop = button("Stop tracking on this phone"); root.addView(stop); stop.setOnClickListener(v -> stopService(new Intent(this, LocationService.class)));
    ...
    private void requestOtp() { final String value = phone.getText().toString().trim(); if (value.isEmpty()) { status.setText("Enter your mobile number."); return; } status.setText("Sending OTP…"); try { JSONObject body = new JSONObject(); body.put("phone", value); post("/api/phone/request", body, result -> status.setText(result.optString("demoCode", "OTP sent. Check your phone."))); } catch (Exception e) { status.setText(e.getMessage()); } }
    private void verifyPairing() { status.setText("Pairing device…"); try { JSONObject body = new JSONObject(); body.put("phone", phone.getText().toString().trim()); body.put("code", otp.getText().toString().trim()); post("/api/phone/verify", body, result -> { prefs.edit().putString("device_token", result.getString("deviceToken")).putString("device_link_id", result.getString("deviceLinkId")).apply(); runOnUiThread(() -> { start.setEnabled(true); status.setText("Paired. Allow location, then start tracking."); requestLocationPermissions(); }); }); } catch (Exception e) { status.setText(e.getMessage()); } }
    ...
    private void startTracking() { if (!locationReady()) { requestLocationPermissions(); status.setText("Allow location permission, then tap Start again."); return; } Intent i = new Intent(this, LocationService.class); if (Build.VERSION.SDK_INT >= 26) startForegroundService(i); else startService(i); status.setText("Tracking active. Keep the Veyra notification enabled."); }
    @Override public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] results) { super.onRequestPermissionsResult(requestCode, permissions, results); if ((requestCode == 21 || requestCode == 22) && locationReady()) { status.setText("Location allowed. Starting background tracking…"); startTracking(); } }
